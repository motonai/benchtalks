...
func main() {
	...
	if len(cfg.NATSPeers) > 0 {
		...
		if err != nil {
			...
			//if an operator has configured peers, they expect federation to work. A silent failure would be WAY worse than a crash in that context.
		...
		//Practice-wise, this runs on a clean exit, and it lets NATS know this bench was taken by the tiny giant from the park.
		defer relay.Close()
	}
