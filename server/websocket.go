...
const (
	writeWait  = 10 * time.Second //sending you the message within 10 seconds, otherwise fricc you.
	pongWait   = 60 * time.Second //"are you there? in 60s you won't be either way"
	pingPeriod = 45 * time.Second //"BEFORE YOU'RE GONE though... are you even alive?"
)

//Ξαναπέρνω στο ονειρό μου... μα σιωπή μόνο στο κινητό μου! 🎶

// when you connect to this place via a browser, it's an http request. But if you wanna talk we need to change some things, like the http into ws so it's persistent.
...
//Structs is the way to go since this is working with RAM. Messages traveling over the ws, are JSON and structs give shape with connections.
//since I'm on the server's side, "incoming" messages are the ones that come from browsers and "outgoing" messages are the ones the server sends to the clients subscribed to rooms.
...
//Since I'm on server side, when it receives a new ws connection, there's a certain way it has to handle it, kinda like Po and the cannonball from Kung Fu Panda.
// So, it changes from http to ws so it's continuous. If that doesn't happen it writes it down and goes bye-bye.
// Then a client/seat is created with random id, attaching the ws connection to it and giving them mouths and ears.

// Τώρα μου μιλάει, τώρα με φιλάει! Τώρα μου λέει πως τρελά με αγαπάει... τώρα με αγκαλιάζει, τα άστρα όλα μου τάζει, την ζωή μου όλη την αλλάζει! (ΔΕΝ) Πεθαίνω... (λόγω pingPeriod) 🎶
...
//The ears and mouths are the read and write pumps. server launches them as soon as the ws connection happens and returns ASAP.
//The go routines, are working continuously in the bg. As soon as the http handling is done the go routines "own" the client/seat on the bench.
